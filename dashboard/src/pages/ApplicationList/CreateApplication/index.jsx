...
class TableList extends React.Component {
  ...
  componentWillMount(){
    ...
    if(this.props.location.state){
      let WorkloadType = this.props.location.state.WorkloadType || 'Deployment';
      let TraitType = this.props.location.state.TraitType || 'Autoscaling';
      let activeStep = this.props.location.state.activeStep || 0;
      let tempState1 = Object.assign({}, this.state.step1InitialValues, { WorkloadType: WorkloadType });
      let tempState2 = Object.assign({}, this.state.step2InitialValues, { TraitType0: TraitType });
      this.setState({
        current: activeStep,
  ...
  onFinishStep1 = values => {
    console.log(values,this.formRefStep1);
    this.setState({
      current: 1,
  ...
  addMore = (e) =>{
    e.preventDefault()
    this.setState({
      traitNum: this.state.traitNum.concat([1])
    })
  };
  render() {
    ...
    if (current == 0) {
      currentDetail = (
        <div>
          <div className="minBox">
            <Form initialValues={step1InitialValues} labelAlign='left' {...layout} ref={this.formRefStep1} name="control-ref" onFinish={this.onFinishStep1} style={{ width: '60%' }}>
              ...
              <div className="buttonBox">
                <Button type='primary' className="floatRight" htmlType="submit">Next</Button>
        ...
        <div>
          <div className="minBox">
            <div style={{ padding: '0px 48px 0px 16px', width: '60%'  }}>
              <p style={{ fontSize: '18px', lineHeight: '32px' }}>Name:<span>{step1InitialValues.WorkloadName}</span></p>
            </div>
            <Form initialValues={step2InitialValues} labelAlign='left' {...layout} ref={this.formRefStep2} name="control-ref" onFinish={this.onFinishStep2} style={{ width: '60%' }}>
              <div style={{ border: '1px solid #eee', padding: '16px 48px 16px 16px' }}>
                  <p className="title">{step1InitialValues.WorkloadType}</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>apps/v1</span>
                    <span style={{ color: '#1890ff', cursor: 'pointer', display: this.state.isShowMore?'none':'black' }} onClick={this.changeShowMore}>more...</span>
                  </div>
                  {
                    this.state.isShowMore
                    ?
                    <div>
                      <p className="title" style={{ marginTop: '16px' }}>Settings:</p>
                      <Row>
                        <Col span="8">
              ...
              </div>
              {
                traitNum.map((item,index)=>{
                  return (
                    <div style={{ border: '1px solid #eee', margin: '16px 0px 8px' }} key={index}>
                      <div style={{ padding: '16px 48px 0px 16px'  }}>
                        <Form.Item
                          name={'TraitType'+index}
                          label="Trait"
                        >
                          <Select
              ...
              <button style={{ marginTop: '16px' }} onClick={this.addMore}>Add More...</button>
              <div className="buttonBox">
                <Button type='primary' className="floatRight" htmlType="submit">Next</Button>
                <Button className="floatRightGap" onClick={this.gotoStep1}>Back</Button>
              </div>
            </Form>
          </div>
        </div>
      ...
      currentDetail = (
        <div>
          <div className="minBox">
            <p>Name:<span>{step1InitialValues.WorkloadName}</span></p>
            <Row>
              <Col span="11">
              ...
              <Col span="10">
                {
                  traitNum.map((item,index)=>{
                    return (
                      <div className="summaryBox" key={index}>
                        <Row>
                          <Col span="22">
          ...
          <div className="buttonBox">
            <Link to="/ApplicationList">
              <Button type='primary' className="floatRight">Confirm</Button>
            </Link>
            <Button className="floatRightGap" onClick={this.gotoStep2}>Back</Button>
          </div>
        </div>
      );
    }
