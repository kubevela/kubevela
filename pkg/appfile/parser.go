...
func (p *Parser) GenerateApplicationConfiguration(app *Appfile, ns string) (*v1alpha2.ApplicationConfiguration,
	...
	for _, wl := range app.Workloads {
		...
		requiredSecrets, err := parseWorkloadK8sSecretSchema(ctx, p.client, ns, wl)
		if err != nil {
			return nil, nil, err
		}
...
func generateComponentFromCUEModule(c client.Client, wl *Workload, appName, revision, ns string) (*v1alpha2.Component, *v1alpha2.ApplicationConfigurationComponent, error) {
	...
	if wl.IsCloudResourceProducer() {
		outputSecretName, err = SetOutputSecretNames(wl)
		if err != nil {
			return nil, nil, err
		}
	}
...
func PrepareProcessContext(k8sClient client.Client, wl *Workload, applicationName, revision, namespace, outputSecretName string) (process.Context, error) {
	pCtx := process.NewContext(namespace, wl.Name, applicationName, revision, outputSecretName, wl.RequiredSecrets)
...
// SetOutputSecretNames set all secret names, which are generated by cloud resource, to context
func SetOutputSecretNames(workloads *Workload) (string, error) {
...
func parseWorkloadK8sSecretSchema(ctx context.Context, c client.Client, namespace string, wl *Workload) ([]process.RequiredSecrets, error) {
	...
	tag := utils.K8sSecretSchemaTag
	for k, v := range schema.Properties {
		description := v.Value.Description
		if strings.Contains(description, tag) {
			contextName := strings.Split(description, tag)[1]
			contextName = strings.TrimSpace(contextName)
			secretNameInterface, err := getComponentSetting(k, wl.Params)
			if err != nil {
				return nil, err
			}
			secretName := fmt.Sprint(secretNameInterface)
...
func extractSecret(ctx context.Context, c client.Client, namespace, name string) (map[string]interface{}, error) {
	...
	if err := c.Get(ctx, client.ObjectKey{Name: name, Namespace: namespace}, &secret); err != nil {
		return nil, fmt.Errorf("failed to get secret %s from namespace %s which is required by the component: %v",
			name, namespace, err)
	}
	...
	if len(secretData) == 0 {
		return nil, fmt.Errorf("data in secret %s from namespace %s isn't avaialble", name, namespace)
	}
...
func (wl *Workload) IsCloudResourceProducer() bool {
	var flag bool
	_, flag = wl.Params[process.OutputSecretName]
	return flag
}
