...
func config(plural, kind, version string) CRD {
	return CRD{
		Plural:           plural,
		GroupVersionKind: schema.GroupVersionKind{Group: "config.oam.dev", Version: version, Kind: kind},
	}
}
...
var Required = []CRD{
	...
	// Their controllers are set up unconditionally and their informers watch
	// these resources, so an absent CRD stops the manager rather than costing a
	// feature.
	config("configs", "Config", "v1alpha1"),
	config("configtemplates", "ConfigTemplate", "v1alpha1"),
}
