...
func RenderComponent(ctx context.Context, params *oamprovidertypes.OAMParams[cue.Value]) (cue.Value, error) {
	...
	workload, traits, err := h.RenderComponent(ctx, params.Appfile, *comp, patcher, clusterName, overrideNamespace, env)
	if err != nil {
		return cue.Value{}, err
	}
...
func ApplyComponent(ctx context.Context, params *oamprovidertypes.OAMParams[cue.Value]) (cue.Value, error) {
	...
	workload, traits, healthy, err := h.ApplyComponent(ctx, params.Appfile, *comp, patcher, clusterName, overrideNamespace, env)
	if err != nil {
		return cue.Value{}, err
	}
...
func lookUpCompInfo(v cue.Value) (*common.ApplicationComponent, cue.Value, string, string, string, error) {
	...
	if !compSettings.Exists() {
		return nil, cue.Value{}, "", "", "", workflowerrors.LookUpNotFoundErr("value")
	}
	...
	env, err := v.LookupPath(cue.ParsePath("env")).String()
	if err != nil {
		env = ""
	}
