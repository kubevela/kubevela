...
func (i InfoCalculateCronJob) calculateAndUpdate(ctx context.Context, systemInfo model.SystemInfo) error {
	...
	appCount, topKComp, topKTrait, topWorkflowStep, err := i.calculateAppInfo(ctx)
	if err != nil {
		return err
	}
...
func (i InfoCalculateCronJob) calculateAppInfo(ctx context.Context) (int, []string, []string, []string, error) {
	var err error
	var appCount int
	...
	return appCount, topKFrequent(compDef, TopKFrequent), topKFrequent(traitDef, TopKFrequent), topKFrequent(workflowDef, TopKFrequent), nil
}
