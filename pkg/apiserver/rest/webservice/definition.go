...
func (d *definitionWebservice) GetWebService() *restful.WebService {
	...
	ws.Route(ws.GET("/").To(d.listDefinitions).
		...
		Param(ws.QueryParameter("appliedWorkloads", "if specified, query the trait definition applied to the workloads.").DataType("string")).
...
func (d *definitionWebservice) listDefinitions(req *restful.Request, res *restful.Response) {
	definitions, err := d.definitionUsecase.ListDefinitions(req.Request.Context(), req.QueryParameter("envName"), req.QueryParameter("type"), req.QueryParameter("appliedWorkloads"))
	if err != nil {
