...
var _ = Describe("Test integration service", func() {
	...
	It("Test apply a new integration", func() {
		_, err := integrationService.CreateIntegration(context.TODO(), "", v1.CreateIntegrationRequest{
		...
		integration, err := integrationService.CreateIntegration(context.TODO(), "", v1.CreateIntegrationRequest{
			Name:        "alibaba-test",
			Alias:       "Alibaba Cloud",
			Description: "This is a terraform provider",
			Template: v1.NamespacedName{
		...
		Expect(integration.Sensitive).To(Equal(true))
		Expect(integration.Description).To(Equal("This is a terraform provider"))
	...
	It("Test detail a integration", func() {
		_, err := integrationService.GetIntegration(context.TODO(), "", "alibaba-test")
		Expect(err).To(Equal(bcode.ErrSensitiveIntegration))
	})

	It("Test delete a integration", func() {
		Expect(integrationService.DeleteIntegration(context.TODO(), "", "alibaba-test")).To(BeNil())
