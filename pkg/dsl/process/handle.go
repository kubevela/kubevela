...
type Context interface {
	...
	InsertSecrets(outputSecretName string, requiredSecrets []RequiredSecrets, envMappings []v1.EnvVar)
}
...
func (ctx *templateContext) ExtendedContextFile() string {
	...
	if ctx.envMappings != nil {
		data, _ := json.Marshal(ctx.envMappings)
		context += fmt.Sprintf("\n%s: %s: %s", mycue.ParameterTag, "env", string(data))
	}
	return context
}
...
func (ctx *templateContext) InsertSecrets(outputSecretName string, requiredSecrets []RequiredSecrets, envMappings []v1.EnvVar) {
	if outputSecretName != "" {
