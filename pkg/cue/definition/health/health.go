...
func GetStatus(templateContext map[string]interface{}, request *StatusRequest) (*StatusResult, error) {
	message, _ := getStatusMessage(templateContext, request.Custom, request.Parameter)
	statusMap, _ := getStatusMap(templateContext, request.Status, request.Parameter)
	return &StatusResult{
		Message: message,
		Status:  statusMap,
	}, nil
...
outer:
	for iter.Next() {
		...
		if len(label) >= 32 {
			klog.Warningf("status field label %s is too long, skipping", label)
