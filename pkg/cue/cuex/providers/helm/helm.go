...
// NewProvider creates a new Helm provider
...
func (p *Provider) fetchChart(ctx context.Context, params *ChartSourceParams) (*chart.Chart, error) {
	sourceType := detectChartSourceType(params.Source)
	
	// Create cache key
	cacheKey := fmt.Sprintf("helm-chart-%s-%s-%s", sourceType, params.Source, params.Version)
	
	// Check cache
	if cached := p.cache.Get(cacheKey); cached != nil {
		if ch, ok := cached.(*chart.Chart); ok {
			klog.V(4).Infof("Using cached chart for %s", cacheKey)
			return ch, nil
		}
	}
	...
	switch sourceType {
	case "oci":
		ch, err = p.fetchOCIChart(ctx, params)
	case "url":
		ch, err = p.fetchURLChart(ctx, params)
	case "repo":
		ch, err = p.fetchRepoChart(ctx, params)
	default:
	...
	// Cache the chart
	p.cache.Put(cacheKey, ch, 24*time.Hour)
...
func Render(ctx context.Context, params *providers.Params[RenderParams]) (*providers.Returns[RenderReturns], error) {
	...
	// Fetch the chart
	chart, err := p.fetchChart(ctx, &renderParams.Chart)
	if err != nil {
