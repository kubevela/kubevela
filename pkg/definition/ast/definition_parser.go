...
func DecodeMetadata(field *ast.Field) error {
	if err := unmarshalStatusField(field); err != nil {
		return err
	}
	return nil
}

func marshalStatusField(field *ast.Field) error {
...
func unmarshalStatusField(field *ast.Field) error {
	if statusField, ok := GetFieldByPath(field, status); ok {
		basicLit, ok := statusField.Value.(*ast.BasicLit)
		if !ok || basicLit.Kind != token.STRING {
			return fmt.Errorf("status field is not a string literal")
		}
...
func validateStatusField(sl *ast.StructLit) error {
	for _, elt := range sl.Elts {
		f, ok := elt.(*ast.Field)
		if !ok {
			return fmt.Errorf("status contains non-field element")
		}
		...
		case *ast.BasicLit,
			...
			*ast.BinaryExpr:
			continue
		default:
			return fmt.Errorf("status field %q contains unsupported expression type %T", label, f.Value)
		}
	}
	return nil
}
