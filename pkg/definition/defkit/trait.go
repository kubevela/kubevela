...
type TraitCUEGenerator struct {
	indent string
	// imports holds the imports declared explicitly via WithImports; they apply
	// to every definition this generator produces.
	imports []string
	// detected holds the imports inferred from the trait currently being
	// generated. It is reset per detection pass so one trait's imports never
	// leak into the next trait generated by the same instance.
	detected []string
}
...
// addImportIfMissing records a detected import unless it is already declared
// explicitly or has been detected for the trait being generated.
func (g *TraitCUEGenerator) addImportIfMissing(imp string) {
	...
	for _, existing := range g.detected {
		if existing == imp {
			return
		}
	}
	g.detected = append(g.detected, imp)
}
...
func (g *TraitCUEGenerator) detectRequiredImports(t *TraitDefinition) {
	...
	// scan is a fresh generator, so everything it found sits in its detected set.
	for _, imp := range scan.detected {
		g.addImportIfMissing(imp)
	}
}
...
func (g *TraitCUEGenerator) GenerateFullDefinition(t *TraitDefinition) string {
	...
	if imports := g.allImports(); len(imports) > 0 {
...
func (g *TraitCUEGenerator) GenerateDefinitionWithRawTemplate(t *TraitDefinition, rawTemplate string) string {
	...
	// Write imports if any. This path runs no detection, so only the explicitly
	// declared imports apply; anything detected for an earlier definition must
	// not be carried over here.
	if len(g.imports) > 0 {
		sb.WriteString("import (\n")
