...
type CUEGenerator struct {
	indent string
	// imports holds the imports the caller declared explicitly via WithImports.
	// They persist for the lifetime of the generator and are emitted for every
	// definition it generates.
	imports []string
	// detected holds the imports inferred from the definition currently being
	// generated. It is reset per Generate* call so that one definition's imports
	// never leak into the next one generated by the same instance.
	detected []string
}
...
// The detected set is rebuilt from scratch on every call so that generating one
// definition never carries imports over into the next.
...
// explicitly or has been detected for the definition being generated.
