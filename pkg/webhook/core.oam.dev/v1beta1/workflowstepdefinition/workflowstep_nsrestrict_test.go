...
// A malformed glob would otherwise deny every namespace at render time, nowhere
// near where the typo was made, so the handler has to catch it on write.
func TestHandleValidatesNamespaceRestrictions(t *testing.T) {
	...
	build := func(spec []string, annotation string) admission.Request {
		def := &v1beta1.WorkflowStepDefinition{
			TypeMeta: metav1.TypeMeta{Kind: "WorkflowStepDefinition", APIVersion: "core.oam.dev/v1beta1"},
			ObjectMeta: metav1.ObjectMeta{
				Name: "restricted-step", Namespace: "vela-system",
			},
			Spec: v1beta1.WorkflowStepDefinitionSpec{Restrictions: nsRestrictions(spec)},
		}
		if annotation != "" {
		...
		return admission.Request{AdmissionRequest: admissionv1.AdmissionRequest{
			...
			Resource: metav1.GroupVersionResource{
				Group: workflowStepDefGVR.Group, Version: workflowStepDefGVR.Version, Resource: workflowStepDefGVR.Resource,
			},
