...
// extendsIndex indexes a ComponentDefinition by the definition it extends, so
...
// revision, so a child on `webservice@v3` still wakes when `webservice`
// changes.
func indexExtends(ctx context.Context, mgr ctrl.Manager) error {
	return mgr.GetFieldIndexer().IndexField(ctx, &v1beta1.ComponentDefinition{}, extendsIndex,
	...
	return func(ctx context.Context, obj client.Object) []reconcile.Request {
		...
		for len(frontier) > 0 {
			...
			for _, name := range frontier {
				children := &v1beta1.ComponentDefinitionList{}
				if err := cli.List(ctx, children,
					...
					klog.ErrorS(err, "listing what extends a component definition, so its descendants may not refresh",
