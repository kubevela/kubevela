...
`,
		...
		Expect(additionalCtx["componentCount"]).Should(Equal(float64(2)))
	})
...
`,
	...
	It("Test applyRenderedPolicyResult applies cached transforms correctly", func() {
		...
		resultCtx, err := handler.applyRenderedPolicyResult(monCtx, app, renderedResult)
		Expect(err).Should(BeNil())
		...
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		Expect(app.Status.AppliedGlobalPolicies[0].Name).Should(Equal("cached-policy"))
		Expect(app.Status.AppliedGlobalPolicies[0].Applied).Should(BeTrue())
	})
	...
	It("Test applyRenderedPolicyResult skips disabled policies", func() {
		...
		// Apply the rendered result
		_, err := handler.applyRenderedPolicyResult(monCtx, app, renderedResult)
		Expect(err).Should(BeNil())
		...
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		Expect(app.Status.AppliedGlobalPolicies[0].Name).Should(Equal("disabled-policy"))
		Expect(app.Status.AppliedGlobalPolicies[0].Applied).Should(BeFalse())
		Expect(app.Status.AppliedGlobalPolicies[0].Reason).Should(Equal("enabled=false"))
	})
	...
	It("Test applyRenderedPolicyResult tracks label changes in status", func() {
		...
		// Verify status tracks what labels were added
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		Expect(app.Status.AppliedGlobalPolicies[0].Applied).Should(BeTrue())
		Expect(app.Status.AppliedGlobalPolicies[0].AddedLabels).Should(HaveLen(2))
		Expect(app.Status.AppliedGlobalPolicies[0].AddedLabels["added-by"]).Should(Equal("policy"))
		Expect(app.Status.AppliedGlobalPolicies[0].AddedLabels["environment"]).Should(Equal("test"))
	})
	...
	It("Test applyRenderedPolicyResult tracks annotation changes in status", func() {
		...
		// Verify status tracks what annotations were added
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		Expect(app.Status.AppliedGlobalPolicies[0].Applied).Should(BeTrue())
		Expect(app.Status.AppliedGlobalPolicies[0].AddedAnnotations).Should(HaveLen(2))
		Expect(app.Status.AppliedGlobalPolicies[0].AddedAnnotations["policy.oam.dev/applied"]).Should(Equal("true"))
		Expect(app.Status.AppliedGlobalPolicies[0].AddedAnnotations["policy.oam.dev/version"]).Should(Equal("v1.0"))
	})
	...
	It("Test applyRenderedPolicyResult tracks additionalContext in status", func() {
		...
		// Verify status tracks additionalContext
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		Expect(app.Status.AppliedGlobalPolicies[0].Applied).Should(BeTrue())
		Expect(app.Status.AppliedGlobalPolicies[0].AdditionalContext).Should(HaveLen(3))
		Expect(app.Status.AppliedGlobalPolicies[0].AdditionalContext["policyApplied"]).Should(Equal("context-policy"))
		Expect(app.Status.AppliedGlobalPolicies[0].AdditionalContext["timestamp"]).Should(Equal("2024-01-01"))
		Expect(app.Status.AppliedGlobalPolicies[0].AdditionalContext["configHash"]).Should(Equal("abc123"))
	})
	...
	It("Test applyRenderedPolicyResult tracks spec modification in status", func() {
		...
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		Expect(app.Status.AppliedGlobalPolicies[0].Applied).Should(BeTrue())
		Expect(app.Status.AppliedGlobalPolicies[0].SpecModified).Should(BeTrue())
	})
	...
	It("Test applyRenderedPolicyResult tracks all changes together", func() {
		...
		// Verify status tracks all changes
		Expect(app.Status.AppliedGlobalPolicies).Should(HaveLen(1))
		policy := app.Status.AppliedGlobalPolicies[0]
		Expect(policy.Applied).Should(BeTrue())
		Expect(policy.AddedLabels).Should(HaveLen(1))
		Expect(policy.AddedLabels["team"]).Should(Equal("platform"))
		Expect(policy.AddedAnnotations).Should(HaveLen(1))
		Expect(policy.AddedAnnotations["policy.oam.dev/applied"]).Should(Equal("true"))
		Expect(policy.SpecModified).Should(BeTrue())
		Expect(policy.AdditionalContext).Should(HaveLen(1))
		Expect(policy.AdditionalContext["applied"]).Should(Equal(true))
	})
})
