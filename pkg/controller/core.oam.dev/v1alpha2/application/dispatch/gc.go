...
type GarbageCollector interface {
	GarbageCollect(ctx *common2.ReconcileContext, oldRT, newRT *v1beta1.ResourceTracker, legacyRTs []*v1beta1.ResourceTracker) error
}
...
func (h *GCHandler) GarbageCollect(ctx *common2.ReconcileContext, oldRT, newRT *v1beta1.ResourceTracker, legacyRTs []*v1beta1.ResourceTracker) error {
	h.oldRT = oldRT
	h.newRT = newRT
	if err := h.validate(); err != nil {
		return err
	}
	ctx.Info("Garbage collect for application", "old", h.oldRT.Name, "new", h.newRT.Name)
	...
	if err := h.c.Delete(ctx, h.oldRT); err != nil && !kerrors.IsNotFound(err) {
		ctx.Error(err, "Failed to delete resource tracker", "name", h.oldRT.Name)
		return errors.Wrapf(err, "cannot delete resource tracker %q", h.oldRT.Name)
	}
	ctx.Info("Successfully GC a resource tracker and its resources", "name", h.oldRT.Name)
...
func (h *GCHandler) handleResourceSkipGC(ctx *common2.ReconcileContext, u *unstructured.Unstructured, oldRt *v1beta1.ResourceTracker) (bool, error) {
	...
	if err := h.c.Get(ctx, types.NamespacedName{Namespace: res.GetNamespace(), Name: res.GetName()}, res); err != nil {
		if !kerrors.IsNotFound(err) {
			ctx.Error(err, "handleResourceSkipGC faied cannot get res kind ", res.GetKind(), "namespace", res.GetNamespace(), "name", res.GetName())
			return false, err
		}
