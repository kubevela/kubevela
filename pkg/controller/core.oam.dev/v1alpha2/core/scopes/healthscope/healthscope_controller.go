...
func (r *Reconciler) Reconcile(req reconcile.Request) (reconcile.Result, error) {
	klog.InfoS("Reconciling", "request", req)
	...
	klog.InfoS("HealthScope Information", "uid", hs.GetUID(), "version", hs.GetResourceVersion())

	scopeCondition, wlConditions := r.GetScopeHealthStatus(ctx, hs)
	klog.InfoS("Successfully ran health check", "scope", hs.Name)
	...
	for _, workloadRef := range scopeWLRefs {
		go func(resRef runtimev1alpha1.TypedReference) {
			...
			if wlHealthCondition != nil {
				klog.InfoS("get health condition from health check trait ", "workload", resRef, "healthCondition", wlHealthCondition)
			...
			for _, checker := range r.checkers {
				...
				if wlHealthCondition != nil {
					klog.InfoS("get health condition from built-in checker", "workload", resRef, "healthCondition", wlHealthCondition)
			...
			klog.InfoS("get unknown workload", "workload", resRef)
