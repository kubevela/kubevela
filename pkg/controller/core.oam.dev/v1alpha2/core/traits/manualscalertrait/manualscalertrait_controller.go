...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	workload, err := util.FetchWorkload(ctx, r.Client, mLog, &manualScalar)
	if err != nil {
		r.record.Event(eventObj, event.Warning(util.ErrLocateWorkload, err))
		return util.ReconcileWaitResult, util.PatchCondition(
			ctx, r, &manualScalar, cpv1.ReconcileError(errors.Wrap(err, util.ErrLocateWorkload)))
	}
	...
	if err != nil {
		...
		return util.ReconcileWaitResult, util.PatchCondition(ctx, r, &manualScalar,
			cpv1.ReconcileError(errors.New(util.ErrFetchChildResources)))
	}
	...
	return ctrl.Result{}, util.PatchCondition(ctx, r, &manualScalar, cpv1.ReconcileSuccess())
}
...
func (r *Reconciler) scaleResources(ctx context.Context, mLog logr.Logger,
	...
	if err != nil {
		return util.ReconcileWaitResult,
			util.PatchCondition(ctx, r, &manualScalar, cpv1.ReconcileError(errors.Wrap(err, errQueryOpenAPI)))
	}
	...
	for _, res := range resources {
		if locateReplicaField(document, res) {
			...
			if err != nil {
				...
				return util.ReconcileWaitResult,
					util.PatchCondition(ctx, r, &manualScalar, cpv1.ReconcileError(errors.Wrap(err, errPatchTobeScaledResource)))
			}
			...
			if err := r.Patch(ctx, res, resPatch, client.FieldOwner(manualScalar.GetUID())); err != nil {
				...
				return util.ReconcileWaitResult,
					util.PatchCondition(ctx, r, &manualScalar, cpv1.ReconcileError(errors.Wrap(err, errScaleResource)))
			}
	...
	if !found {
		...
		return util.ReconcileWaitResult,
			util.PatchCondition(ctx, r, &manualScalar, cpv1.ReconcileError(errors.New(errScaleResource)))
	}
