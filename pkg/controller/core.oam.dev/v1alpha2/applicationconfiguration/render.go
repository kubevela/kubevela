...
func (r *components) Render(ctx context.Context, ac *v1alpha2.ApplicationConfiguration) ([]Workload, *v1alpha2.DependencyStatus, error) {
	...
	isControlledByApp := isControlledByApp(ac)
	...
	for _, acc := range ac.Spec.Components {
		w, err := r.renderComponent(ctx, acc, ac, isControlledByApp, rollingComponents[acc.RevisionName], dag)
		if err != nil {
			return nil, nil, err
		}
...
func (r *components) renderComponent(ctx context.Context, acc v1alpha2.ApplicationConfigurationComponent,
	ac *v1alpha2.ApplicationConfiguration, isControlledByApp, isCompRolling bool, dag *dag) (*Workload, error) {
	if acc.RevisionName != "" {
	...
	klog.InfoS("render a component", "component name", acc.ComponentName, "component revision", acc.RevisionName,
		"is generated by application", isControlledByApp, "is a newly changed component", isCompRolling)
	...
	util.RemoveAnnotations(w, []string{oam.AnnotationNewAppConfig, oam.AnnotationRollingComponent})
	...
	for _, ct := range acc.Traits {
		...
		util.RemoveAnnotations(t, []string{oam.AnnotationNewAppConfig, oam.AnnotationRollingComponent})
	...
	if !isControlledByApp {
		...
		if isCompRolling {
			// we have a special logic to emit the workload disabled so that the rollout process can take over
			// this is the only place the appConfig controller is involved in the rollout process
