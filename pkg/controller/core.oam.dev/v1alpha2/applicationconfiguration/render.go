...
func (r *components) Render(ctx context.Context, ac *v1alpha2.ApplicationConfiguration) ([]Workload, *v1alpha2.DependencyStatus, error) {
	...
	for _, acc := range ac.Spec.Components {
		if acc.RevisionName != "" {
			acc.ComponentName = ctrlutil.ExtractComponentName(acc.RevisionName)
		}
	...
	if !isControlledByApp {
		...
		if c.Spec.Helm != nil {
			...
			if err != nil {
				...
				return nil, ctrlutil.RequeueSoonAfterErr(errors.WithMessage(err, "cannot get the workload created by a Helm module"))
			}
			...
			revision, err := ctrlutil.ExtractRevision(acc.RevisionName)
			if err != nil {
				return nil, err
			}
