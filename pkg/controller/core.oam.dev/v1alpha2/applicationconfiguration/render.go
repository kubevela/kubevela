...
func (r *components) Render(ctx context.Context, ac *v1alpha2.ApplicationConfiguration) ([]Workload, *v1alpha2.DependencyStatus, error) {
	...
	var rolloutTemplate string
	var exist bool
	if rolloutTemplate, exist = ac.GetAnnotations()[oam.AnnotationAppRollout]; exist {
		// we only care about the new components when there is rolling out, no matter if it's true or not
		if anc, exist := ac.GetAnnotations()[oam.AnnotationRollingComponent]; exist {
			// the new components annotation contains all the changed components in the application
			for _, revisionName := range strings.Split(anc, common.RollingComponentsSep) {
				rollingComponents[revisionName] = true
			}
		}
	}

	for _, acc := range ac.Spec.Components {
		w, err := r.renderComponent(ctx, acc, ac, isControlledByApp(ac), rollingComponents[acc.RevisionName],
			rolloutTemplate == strconv.FormatBool(true), dag)
		if err != nil {
			return nil, nil, err
		}
...
func (r *components) renderComponent(ctx context.Context, acc v1alpha2.ApplicationConfigurationComponent,
	ac *v1alpha2.ApplicationConfiguration, isControlledByApp, isChanged, isRolloutTemplate bool, dag *dag) (*Workload, error) {
	if acc.RevisionName != "" {
	...
	klog.InfoS("render a component", "component name", acc.ComponentName, "component revision", acc.RevisionName,
		"is generated by application", isControlledByApp, "is a changed component", isChanged,
		"is the appConfig a rollout template", isRolloutTemplate)
	...
	if !isControlledByApp {
		...
		if isChanged && isRolloutTemplate {
			// we have a special logic to emit the workload disabled for the first time so that the rollout
