...
func (c *DeploymentController) verifyRolloutBatchReplicaValue(totalReplicas int32) error {
	...
	err := c.verifySumOfBatchSizes(totalReplicas)
	if err != nil {
		return err
	}
	return nil
...
func (c *DeploymentController) verifySumOfBatchSizes(totalReplicas int32) error {
	// if not set, the sum of all the batch sizes minus the last batch cannot be more than the totalReplicas
	totalRollout := 0
	for i := 0; i < len(c.rolloutSpec.RolloutBatches)-1; i++ {
		rb := c.rolloutSpec.RolloutBatches[i]
	...
	lastBatch := c.rolloutSpec.RolloutBatches[len(c.rolloutSpec.RolloutBatches)-1]
