...
func TestResourceKeeperGarbageCollect(t *testing.T) {
	...
	addConfigMapToRT := func(i int, gen int64, compID int) {
		...
		if _, exists := crMaps[compID]; !exists {
			...
			r.NoError(resourcetracker.RecordManifestsInResourceTracker(ctx, cli, crRT, []*unstructured.Unstructured{obj}, nil, true))
		}
		r.NoError(resourcetracker.RecordManifestsInResourceTracker(ctx, cli, _rt, []*unstructured.Unstructured{cmMaps[i]}, nil, true))
	}
...
func TestResourceKeeperGarbageCollectForSequential(t *testing.T) {
	MarkWithProbability = 1.0
	r := require.New(t)
	cli := fake.NewClientBuilder().WithScheme(common.Scheme).Build()
	ctx := context.Background()
	...
	addConfigMapToRT := func(i int, gen int64, compID int, dependsOn []string) {
		_rt := rtMaps[gen]
		if _, exists := cmMaps[i]; !exists {
			cm := &unstructured.Unstructured{}
			cm.SetName(fmt.Sprintf("cm-%d", i))
	...
	opts := []GCOption{DisableLegacyGCOption{}}
	// delete app, gc resource by the reversal order of the creation
	createRT(1)
	addConfigMapToRT(1, 1, 1, []string{"comp-2"})
	addConfigMapToRT(2, 1, 2, nil)
	rk := createRK(1, false)
	dt := v12.Now()
	rk.app.SetDeletionTimestamp(&dt)
	finished, _, err := rk.GarbageCollect(ctx, opts...)
