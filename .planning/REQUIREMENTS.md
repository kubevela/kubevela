...
- [x] **A2**: `StructParam.Fields()` and `OneOfVariant.Fields()` renamed to `WithFields()` — align with ArrayParam and MapParam
- [x] **A3**: `StructField.ArrayOf()` renamed to `Of()` — align with ArrayParam.Of(); `Array` prefix is redundant
- [x] **A4**: `EnumParam.Values()` renamed to `Enum()` — align with StringParam.Enum() and StructField.Enum()
...
| A4 | Phase 5 — High-Impact Renames + Downstream | Complete |
| A2 | Phase 5 — High-Impact Renames + Downstream | Complete |
...
*Last updated: 2026-03-06 after phase 05-01 (A4, A2 complete)*
