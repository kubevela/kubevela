...
| `optimize.informerCache`                          | if enabled, informer cache will be optimized by removing unnecessary fields will be filtered                                                                                                                                     | `true`  |
| `optimize.applicationRevisionDefinitionStorage`   | if enabled, application revision definition cache will be optimized by using reducing duplicated definition storage                                                                                                              | `true`  |
| `optimize.disableWorkflowContextConfigMapCache`   | if enabled, disable the workflow context's configmap informer cache                                                                                                                                                              | `true`  |
