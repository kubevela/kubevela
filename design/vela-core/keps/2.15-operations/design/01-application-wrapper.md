...
| `override`, `replication`, `take-over` | not exposed | n/a |
    ...
    - name: backup-job
      ...
      properties:
        ...
        env:
          ...
          DEST_BUCKET: '$(source["backup-archive"].bucket)'
          DEST_PREFIX: '$(context.name)'
...
spec:
  components:
    - name: backup-job                     # from the template, expressions resolved
      type: task
      properties:
        image: my-org/backup-s3:1.4.0
        env:
          SRC_BUCKET:  payments-db-prod-8f2a
          DEST_BUCKET: acme-archive
          DEST_PREFIX: payments-db
