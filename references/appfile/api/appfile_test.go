...
`
	...
	ac3 := ac1.DeepCopy()
	ac3.Spec.Components[0].Kind = "withconfig"
		...
		t.Run(caseName, func(t *testing.T) {
			...
			for _, comp := range application.Spec.Components {
				...
				for idx, expcomp := range c.want.app.Spec.Components {
					...
					found = true
					assert.Equal(t, comp.Kind, c.want.app.Spec.Components[idx].Kind)
					...
					for tidx, tr := range comp.Traits {
						assert.Equal(t, tr.Kind, c.want.app.Spec.Components[idx].Traits[tidx].Kind)
