...
func AuthCommandGroup(f velacmd.Factory, order string, streams util.IOStreams) *cobra.Command {
	...
	cmd.AddCommand(NewRevokePrivilegesCommand(f, streams))
	return cmd
}
...
// RevokePrivilegesOptions options for revoke privileges
type RevokePrivilegesOptions struct {
	auth.Identity
	KubeConfig      string
	GrantNamespaces []string
	GrantClusters   []string
...
// Complete .
func (opt *RevokePrivilegesOptions) Complete(f velacmd.Factory, cmd *cobra.Command) {
	if opt.KubeConfig != "" {
		identity, err := auth.ReadIdentityFromKubeConfig(opt.KubeConfig)
		cmdutil.CheckErr(err)
		opt.Identity = *identity
...
// Validate .
func (opt *RevokePrivilegesOptions) Validate(f velacmd.Factory, cmd *cobra.Command) error {
	if opt.User == "" && len(opt.Groups) == 0 && opt.ServiceAccount == "" {
		return fmt.Errorf("at least one idenity (user/group/serviceaccount) should be set")
	}
	for _, cluster := range opt.GrantClusters {
...
// Run .
func (opt *RevokePrivilegesOptions) Run(f velacmd.Factory, cmd *cobra.Command) error {
	ctx := cmd.Context()
	var privileges []auth.PrivilegeDescription
	for _, cluster := range opt.GrantClusters {
		for _, namespace := range opt.GrantNamespaces {
	...
	if err := auth.RevokePrivileges(ctx, f.Client(), privileges, &opt.Identity, opt.IOStreams.Out); err != nil {
		return err
	}
	_, _ = fmt.Fprintf(opt.IOStreams.Out, "Privileges revoked.\n")
	return nil
}

var (
	revokePrivilegesLong = templates.LongDesc(i18n.T(`
		Revoke privileges for user

		Revoke privileges previously granted to a user/group/serviceaccount by vela auth
		grant-privileges. By using --for-namespace and --for-cluster, you can revoke the
		privileges for all resources in the specified namespace and cluster. If
		--for-namespace is not set, the cluster-wide privileges will be revoked.

		Set --readonly to match the readonly privileges granted by vela auth grant-privileges
		--readonly. This must match how the privileges were originally granted, otherwise the
		wrong RoleBinding/ClusterRoleBinding will be targeted.

		If multiple identity information are set, all the identity information will be
		unbound from the intended privileges respectively.

		If --kubeconfig is set, the user/serviceaccount information in the kubeconfig will be
		used as the identity to revoke privileges. Groups will be ignored.`))

	revokePrivilegesExample = templates.Examples(i18n.T(`
		# Revoke privileges for User alice in the namespace demo of the control plane
		vela auth revoke-privileges --user alice --for-namespace demo

		# Revoke privileges for User alice in the namespace demo in cluster-1
		vela auth revoke-privileges --user alice --for-namespace demo --for-cluster cluster-1

		# Revoke cluster-scoped privileges for Group org:dev-team in the control plane
		vela auth revoke-privileges --group org:dev-team

		# Revoke privileges for Group org:dev-team and org:test-team in the namespace test on the control plane and managed cluster example-cluster
		vela auth revoke-privileges --group org:dev-team --group org:test-team --for-namespace test --for-cluster local --for-cluster example-cluster

		# Revoke read privileges for ServiceAccount observer in test namespace on the control plane
		vela auth revoke-privileges --serviceaccount observer -n test --for-namespace test --readonly

		# Revoke privileges for identity in kubeconfig in cluster-1
		vela auth revoke-privileges --kubeconfig ./example.kubeconfig --for-cluster cluster-1`))
)

// NewRevokePrivilegesCommand revoke privileges from given identity
func NewRevokePrivilegesCommand(f velacmd.Factory, streams util.IOStreams) *cobra.Command {
	o := &RevokePrivilegesOptions{IOStreams: streams}
	cmd := &cobra.Command{
		Use:                   "revoke-privileges",
		DisableFlagsInUseLine: true,
		Short:                 i18n.T("Revoke privileges for user/group/serviceaccount"),
		Long:                  revokePrivilegesLong,
		Example:               revokePrivilegesExample,
		Annotations: map[string]string{
			types.TagCommandType: types.TypeCD,
		},
		Args: cobra.ExactArgs(0),
	...
	cmd.Flags().StringVarP(&o.User, "user", "u", o.User, "The user to revoke privileges from.")
	cmd.Flags().StringSliceVarP(&o.Groups, "group", "g", o.Groups, "The group to revoke privileges from.")
	cmd.Flags().StringVarP(&o.ServiceAccount, "serviceaccount", "", o.ServiceAccount, "The serviceaccount to revoke privileges from.")
	cmd.Flags().StringVarP(&o.KubeConfig, "kubeconfig", "", o.KubeConfig, "The kubeconfig to revoke privileges from. If set, it will override all the other identity flags.")
	cmd.Flags().StringSliceVarP(&o.GrantClusters, "for-cluster", "", o.GrantClusters, "The clusters privileges to revoke. If empty, the control plane will be used.")
	cmd.Flags().StringSliceVarP(&o.GrantNamespaces, "for-namespace", "", o.GrantNamespaces, "The namespaces privileges to revoke. If empty, cluster-scoped privileges will be revoked.")
	cmd.Flags().BoolVarP(&o.ReadOnly, "readonly", "", o.ReadOnly, "If set, only the readonly privileges will be revoked. This must match how the privileges were granted.")
	cmdutil.CheckErr(cmd.RegisterFlagCompletionFunc(
		"serviceaccount", func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			if strings.TrimSpace(o.User) != "" {
				return nil, cobra.ShellCompDirectiveNoFileComp
